// All of our JavaScript code goes here... 
	...
	document.onkeydown = function(event){
		// console.log(event);
		if (event.keyCode === 37){ // PACMAN MOVE VENSTRE
		...
		else if (event.keyCode === 38){ // PACMAN MOVE OPP
		...
		else if (event.keyCode === 39){ // PACMAN MOVE HØYRE
			if ( map[pacman.y][pacman.x+1] !== 1){
				map[pacman.y][pacman.x] = 3;
				pacman.x = pacman.x + 1;
				map[pacman.y][pacman.x] = 5;
		...
		else if (event.keyCode === 40){ // PACMAN MOVE NED
			if ( map[pacman.y+1][pacman.x] !== 1){
				map[pacman.y][pacman.x] = 3;
				pacman.y = pacman.y + 1;
				map[pacman.y][pacman.x] = 5;
